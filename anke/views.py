...
def ankeView(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            message2 = ("アンケート回答のお礼", f"「{name}」 さま \n ご回答ありがとうございます。\n\nJR田主丸駅構内のカフェ「カパテリア」で「400円分サービス」させていただきます。 \n\nご利用の際はスタッフにこのメールをお見せください。\n\n※メルマガにご登録の方は、今後の配信をお待ちください。", from_email, [from_email])
            try:
...
def ankeExport(request):
    ...
    writer.writerow(['回答日', '手段', '氏名', '住所', '年代', '性別', 'メルマガ', 'Eメールアドレス', '質問１', '質問２', '質問３', '質問４', '質問５', '質問６', '質問7', '質問8', '質問9', '質問10', '質問11', '質問12', '質問13', '質問14', '質問15'])
    for answer in data:
